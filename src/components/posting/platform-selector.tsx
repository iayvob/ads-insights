...
export function PlatformSelector({ selectedPlatforms, onPlatformsChange }: PlatformSelectorProps) {
  ...
  const platforms: Platform[] = [
    {
      ...
      status: !!session?.connectedPlatforms?.instagram ? "connected" : "disconnected",
      characterLimit: 2200,
    ...
    },
    {
      ...
      status: userPlan === 'FREEMIUM' ? "restricted" : (!!session?.connectedPlatforms?.facebook ? "connected" : "disconnected"),
    ...
    },
    {
      ...
      status: userPlan === 'FREEMIUM' ? "restricted" : (!!session?.connectedPlatforms?.twitter ? "connected" : "disconnected"),
      characterLimit: 280,
  ...
  const connectedPlatforms = platforms.filter(p => p.connected && p.status !== 'restricted')
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2 mb-4">
          <Button
            variant="outline"
            size="sm"
            onClick={() => onPlatformsChange(connectedPlatforms.map(p => p.id))}
            className="text-xs"
          >
            Select All
          </Button>
        ...
        {selectedCount > 0 && (
          ...
          >
            ...
            <p className="text-xs text-gray-600 mt-2">
              Your post will be published to {selectedCount} platform{selectedCount > 1 ? 's' : ''}
            </p>
          </motion.div>
        )}
      </CardContent>

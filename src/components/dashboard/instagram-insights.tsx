...
function getAgeGroupColorClass(range: string): string {
  switch (range) {
    case '13-17':
    ...
    >
      ...
      <div className="grid gap-6 md:grid-cols-4">
        ...
        <MetricCard
          ...
          value={Math.round(posts.avgEngagement || 0).toLocaleString()}
          icon={Heart}
        ...
        <MetricCard
          title="Avg. Reach"
          value={Math.round(posts.avgReach || 0).toLocaleString()}
          icon={Eye}
        ...
        <MetricCard
          title="Total Posts"
          value={(posts.totalPosts || 0).toLocaleString()}
          icon={Camera}
          trend={3}
      ...
      <div className="grid gap-6 md:grid-cols-4">
        <MetricCard
          ...
          value={`${(posts.engagementRate || 0).toFixed(2)}%`}
          icon={TrendingUp}
          trend={(posts.engagementRate || 0) > 2 ? 10 : 0}
          description="Total engagement rate"
        />
        <MetricCard
          title="Total Saves"
          value={(posts.totalSaves || 0).toLocaleString()}
          icon={Bookmark}
          trend={(posts.totalSaves || 0) > 0 ? 15 : 0}
          description="Content saved"
        />
        <MetricCard
          title="Profile Views"
          value={(posts.totalProfileViews || 0).toLocaleString()}
          icon={Eye}
          trend={(posts.totalProfileViews || 0) > 0 ? 12 : 0}
          description="Profile visits"
        />
        <MetricCard
          title="Website Clicks"
          value={(posts.websiteClicks || 0).toLocaleString()}
          icon={MousePointer}
          trend={(posts.websiteClicks || 0) > 0 ? 8 : 0}
      ...
      <div className="grid gap-6 md:grid-cols-4">
        <MetricCard
          title="Email Clicks"
          value={(posts.emailClicks || 0).toLocaleString()}
          icon={Mail}
          trend={(posts.emailClicks || 0) > 0 ? 6 : 0}
          description="Email contacts"
        />
        <MetricCard
          title="Phone Clicks"
          value={(posts.phoneCallClicks || 0).toLocaleString()}
          icon={Phone}
          trend={(posts.phoneCallClicks || 0) > 0 ? 4 : 0}
          description="Phone calls"
        />
        <MetricCard
          title="Directions"
          value={(posts.getDirectionsClicks || 0).toLocaleString()}
          icon={Navigation}
          trend={(posts.getDirectionsClicks || 0) > 0 ? 3 : 0}
          description="Get directions"
        />
        {posts.topPost && (
          <MetricCard
            title="Top Post"
            value={Math.round(posts.topPost.engagement || 0).toLocaleString()}
            icon={Heart}
            trend={15}
                ...
                >
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="flex items-center space-x-4 text-xs text-gray-500">
                      ...
                      <span>
                        {Math.round(post.reach || 0).toLocaleString()} reach
                      </span>
                      <span>
                        {Math.round(post.impressions || 0).toLocaleString()}{' '}
                        impressions
                      </span>

...
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, LineChart, Line, PieChart, Pie, Cell, Tooltip, Legend, CartesianGrid, Area, AreaChart } from "recharts"
import { DollarSign, TrendingUp, MousePointer, Percent, Users, Target, ChevronDown, Eye, ArrowRight, ExternalLink } from "lucide-react"
...
export function AdsAnalyticsComponent({ data, platform }: AdsAnalyticsComponentProps) {
  const [dateRange, setDateRange] = useState<'7d' | '30d' | '90d'>('30d');
  const [chartView, setChartView] = useState<'spend' | 'performance'>('performance');
  ...
  const getTrendData = () => {
    ...
    if (data.spendTrend.length > dataPoints) {
      ...
      return data.spendTrend.filter((_, index) => index % step === 0).slice(0, dataPoints);
    }
  ...
  // Calculate ROI/ROAS value
  const roasValue = data.roas;
  
  // Prepare audience data for charts
  const ageGroupData = data.audienceInsights?.ageGroups?.map(item => ({
    name: item.range,
    value: item.percentage
  })) || [];
  
  const genderData = data.audienceInsights?.genders?.map(item => ({
    name: item.gender,
    value: item.percentage
  })) || [];
  
  const locationData = data.audienceInsights?.topLocations?.map(item => ({
    name: item.location,
    value: item.percentage
  })) || [];
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <Badge variant="secondary" className={`bg-gradient-to-r ${colors.gradient} text-white`}>
            Premium Analytics
          </Badge>
          <p className="text-sm text-gray-500 mt-1">
            Advanced ad performance metrics for {platform.charAt(0).toUpperCase() + platform.slice(1)}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            <div className={`w-8 h-8 rounded-full bg-gradient-to-r ${colors.gradient} flex items-center justify-center`}>
              <DollarSign className="w-4 h-4 text-white" />
            </div>
            <span className="text-sm font-medium">Ad Spend</span>
          </div>
          <div className="text-2xl font-bold mt-2">{formatCurrency(data.totalSpend)}</div>
          <p className="text-xs text-gray-500 mt-1">Total for selected period</p>
        </motion.div>
        ...
        >
          ...
          <div className="text-2xl font-bold mt-2">{formatNumber(data.totalImpressions)}</div>
        ...
        >
          ...
          <div className="text-2xl font-bold mt-2">{formatNumber(data.totalClicks)}</div>
        ...
        <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-sm">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <div className="text-3xl font-bold">{formatCurrency(data.cpc)}</div>
        ...
        <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-sm">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <div className="text-3xl font-bold">{formatCurrency(data.cpm)}</div>
              <div className="flex items-center justify-center text-xs text-gray-500 mt-1">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            ...
            <div className="flex gap-2 mt-2 sm:mt-0">
              <Button 
                ...
                className={chartView === 'performance' ? `bg-gradient-to-r ${colors.gradient} text-white` : ""}
              >
                Performance
              </Button>
              <Button 
                ...
                className={chartView === 'spend' ? `bg-gradient-to-r ${colors.gradient} text-white` : ""}
              >
                Spend
              </Button>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendData} margin={{ top: 10, right: 30, left: 20, bottom: 30 }}>
                        ...
                        return (
                          <div className="bg-white p-3 shadow-md rounded-md border border-gray-100">
                            ...
                            <p className="text-blue-600">
                              <span className="font-medium">Impressions:</span> {formatNumber(Number(payload[0].value || 0))}
                            </p>
                            <p className="text-green-600">
                              <span className="font-medium">Clicks:</span> {formatNumber(Number(payload[1].value || 0))}
                            </p>
                          </div>
                        );
                      }
                  ...
                  <Line 
                    ...
                    activeDot={{ r: 8, stroke: "#3b82f6", strokeWidth: 1, fill: "#fff" }}
                  />
                  <Line 
                    name="Clicks"
                    type="monotone" 
                    ...
                    activeDot={{ r: 8, stroke: "#10b981", strokeWidth: 1, fill: "#fff" }}
                  />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData} margin={{ top: 10, right: 30, left: 20, bottom: 30 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
                  <XAxis 
                    dataKey="date" 
                    axisLine={false}
                        ...
                        return (
                          <div className="bg-white p-3 shadow-md rounded-md border border-gray-100">
                            ...
                            <p className="text-purple-600">
                              <span className="font-medium">Spend:</span> {formatCurrency(Number(payload[0].value || 0))}
                            </p>
                          </div>
                        );
                      }
                  ...
                  <Area 
                    ...
                    activeDot={{ r: 8, stroke: colors.primary, strokeWidth: 1, fill: "#fff" }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            )}
                ...
                <tbody>
                  {data.campaigns.map((campaign, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <Badge variant={campaign.status === 'ACTIVE' ? 'default' : 'secondary'} className={campaign.status === 'ACTIVE' ? 'bg-green-500 hover:bg-green-600 text-white' : ''}>
                          {campaign.status}
                        </Badge>
                      </td>
                    </motion.tr>
        ...
        <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-lg overflow-hidden">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Age Groups */}
              <div>
                <h3 className="text-sm font-medium text-gray-700 mb-4 text-center">Age Distribution</h3>
              ...
              {/* Gender Distribution */}
              <div>
                <h3 className="text-sm font-medium text-gray-700 mb-4 text-center">Gender Distribution</h3>
                <div className="h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                      ...
                      >
                        {genderData.map((entry, index) => (
                          <Cell 
                            key={`cell-${index}`} 
                            fill={entry.name.toLowerCase() === 'female' ? '#FF6B9F' : 
                                  entry.name.toLowerCase() === 'male' ? '#4169E1' : '#8884d8'} 
                          />
                        ))}
                      </Pie>
                      <Tooltip content={<CustomPieTooltip />} />
              ...
              {/* Top Locations */}
              <div>
                <h3 className="text-sm font-medium text-gray-700 mb-4 text-center">Top Locations</h3>
                <div className="h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      layout="vertical"
      ...
      {data.topAd && (
        <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-lg overflow-hidden">
          ...
          <CardHeader>
            ...
            <CardDescription>Your best performing advertisement</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-6">
              <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-gray-800">{data.topAd.name}</h3>
                  <p className="text-sm text-gray-500 mb-4">Published on {new Date(data.topAd.date).toLocaleDateString()}</p>
                  
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-white rounded-lg p-4 shadow-sm">
                      <div className="text-sm text-gray-500">Ad Spend</div>
                      <div className="text-xl font-semibold">{formatCurrency(data.topAd.spend)}</div>
                    </div>
                    <div className="bg-white rounded-lg p-4 shadow-sm">
                      <div className="text-sm text-gray-500">Click-Through Rate</div>
                      <div className="text-xl font-semibold">{data.topAd.ctr.toFixed(2)}%</div>
                    </div>
                    <div className="bg-white rounded-lg p-4 shadow-sm">
                      <div className="text-sm text-gray-500">Impressions</div>
                      <div className="text-xl font-semibold">{formatNumber(data.topAd.impressions)}</div>
                    </div>
                    <div className="bg-white rounded-lg p-4 shadow-sm">
                      <div className="text-sm text-gray-500">Clicks</div>
                      <div className="text-xl font-semibold">{formatNumber(data.topAd.clicks)}</div>
                    </div>
                  </div>
                ...
                <div className="w-full md:w-64 flex items-center justify-center">
                  <div className="relative w-full aspect-square max-w-[200px] rounded-full overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                      <div className="font-semibold text-amber-800">Performance</div>
                      <div className="text-3xl font-bold text-orange-700">{data.topAd.ctr.toFixed(1)}x</div>
                      <div className="text-sm text-amber-700 mt-1">above average</div>
                    </div>
                  </div>
                </div>
              </div>

...
function ProfilePageContent() {
  ...
  const { data: session, status, isLoading, update: updateSession } = useSession();
  const {
    profile,
    subscription,
    invoices,
  ...
  useEffect(() => {
    ...
    if (tabFromUrl && ['overview', 'connections', 'subscription', 'settings'].includes(tabFromUrl)) {
    ...
    if (error) {
      ...
      switch (error) {
        ...
        case 'instagram_no_business_accounts':
          errorMessage = 'No Instagram business accounts found. Please ensure your Instagram account is set up as a business account.';
          break;
        default:
  ...
  useEffect(() => {
    if (session?.authenticated) {
      ...
      setAuthStatus({
        facebook: !!session.status?.facebook || !!session.connectedPlatforms?.facebook,
        instagram: !!session.status?.instagram || !!session.connectedPlatforms?.instagram,
        twitter: !!session.status?.twitter || !!session.connectedPlatforms?.twitter,
      ...
      setUserSession({
        facebook: session.connectedPlatforms?.facebook ? {
          accessToken: session.connectedPlatforms.facebook.account_tokens?.access_token || '',
          userId: session.user?.id || '',
          name: session.user?.name || '',
          expiresAt: session.connectedPlatforms.facebook.account_tokens?.expires_at || Date.now() + 3600000,
          email: session.user?.email,
        } : undefined,
        instagram: session.connectedPlatforms?.instagram ? {
          accessToken: session.connectedPlatforms.instagram.account_tokens?.access_token || '',
          userId: session.user?.id || '',
          username: session.user?.name || '',
          expiresAt: session.connectedPlatforms.instagram.account_tokens?.expires_at || Date.now() + 3600000,
        } : undefined,
        twitter: session.connectedPlatforms?.twitter ? {
          accessToken: session.connectedPlatforms.twitter.account_tokens?.access_token || '',
          userId: session.user?.id || '',
          username: session.user?.name || '',
          expiresAt: session.connectedPlatforms.twitter.account_tokens?.expires_at || Date.now() + 3600000,
        } : undefined,
        amazon: session.connectedPlatforms?.amazon ? {
          accessToken: session.connectedPlatforms.amazon.account_tokens?.access_token || '',
          userId: session.user?.id || '',
          username: session.user?.name || '',
          expiresAt: session.connectedPlatforms.amazon.account_tokens?.expires_at || Date.now() + 3600000,
        } : undefined,
      });
  ...
  const handleAuth = async (platform: string) => {
    if (!profile?.id) {
      toast({
        title: "Authentication Error",
        description: "Please ensure your profile is loaded before connecting platforms",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleLogout = async (platform: string) => {
    try {
      ...
      const response = await axios.post(`/api/auth/${platform}/logout`, {
        userId: profile?.id || ""
      }, {
        withCredentials: true,
        headers: {
      ...
      if (response.status === 200) {
        ...
        throw new Error(data.error || data.message || `Failed to logout from ${platform}`);
      }
    } catch (error) {
  ...
  const handleProfileSave = async () => {
    ...
    if (!editedProfile.username || !editedProfile.email || !editedProfile.email.includes('@')) {
      toast({
                  ...
                  <div className="flex items-center space-x-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 flex items-center space-x-2 mt-1">
                        ...
                        <span>Joined {profile && formatDate(profile.createdAt)}</span>
                      </p>
                    </div>
                  </div>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className={`p-4 rounded-lg ${
                          ...
                          <div className={`text-2xl font-bold ${
                            premiumTheme ? "text-yellow-800" : "text-green-800"
                          }`}>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center justify-between p-4 border rounded-lg">
                      ...
                      <PasswordChangeModal userId={profile?.id || ""} userEmail={profile?.email || ""} />
                    </div>
                    ...
                    <div className="flex items-center justify-between p-4 border rounded-lg">
                      ...
                      <Button variant="outline" disabled={true}>Soon...</Button>
                    </div>

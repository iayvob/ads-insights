...
async function handler(request: NextRequest): Promise<NextResponse> {
  ...
  const authUrl = OAuthService.buildTikTokAuthUrl(state, redirectUri)

  logger.info("TikTok auth initiated", { state })

  const response = createSuccessResponse({ authUrl }, "TikTok auth URL generated")
